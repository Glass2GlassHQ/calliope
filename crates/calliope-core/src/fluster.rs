...
/// Heuristic guard against importing a driver-fork reference set. The only
/// safe golden source is canonical Fluster (github.com/fluendo/fluster), whose
/// `result` md5s come from the codec's software reference decoder and reproduce
/// under a plain rawvideo md5. The gstreamer-CI fluster clone ships a
/// `visl_references` set regenerated by the VISL V4L2-stateless driver; those
/// hashes are a real hardware/driver decode, do NOT match a plain rawvideo md5,
/// and scored 0/135 when imported. The suite JSON carries no field naming its
/// reference decoder, so the only signal is the import path: flag it when any
/// component contains "visl". Returns a warning string when it looks forked.
pub fn driver_fork_warning(source: &Path) -> Option<String> {
    let lower = source.to_string_lossy().to_ascii_lowercase();
    lower.contains("visl").then(|| {
        format!(
            "'{}' looks like a VISL driver-fork reference set (path contains \"visl\"); \
             its hashes are not reproducible by a plain rawvideo md5. Import canonical \
             Fluster (github.com/fluendo/fluster) instead.",
            source.display()
        )
    })
}
...
mod tests {
    ...
    #[test]
    fn driver_fork_warning_flags_visl_sources_only() {
        use std::path::Path;
        // a canonical checkout imports silently
        assert!(driver_fork_warning(Path::new("/home/x/fluster/test_suites")).is_none());
        assert!(driver_fork_warning(Path::new("/home/x/fluster/test_suites/h.264")).is_none());
        // the gstreamer-CI visl_references fork is flagged (case-insensitive)
        assert!(
            driver_fork_warning(Path::new("/gst/ci/fluster/visl_references/JVT-AVC_V1.json"))
                .is_some()
        );
        assert!(driver_fork_warning(Path::new("/tmp/VISL_references")).is_some());
    }
